...
module.exports = function(data, callback) {
  ...
  if (lists && lists.length) {
    lists.forEach(function(l) {
      ...
      tmp.shift();
      text = (tmp && tmp.length === 3) ? text.replace(l, tmp.join('/')) : text;
    });
  }
  ...
  text.split('\n|').forEach(function(item) {
    /*
     * FIXME: Each item is not always seperated by ' = ',
     *        sometimes it is ' =', '= ', etc.
     *        It's not possible to split them by simply using '=',
     *        because the item might contains several '=',
     *        like: 'students = 300 + 600 = 900'
     */
    var itemArr = item.split(' = ');

    if (itemArr.length === 2 && itemArr[1].trim() !== '') {
      var item_name    = itemArr[0].trim(),
          item_content = itemArr[1].trim().split('\n')[0];
